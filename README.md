...
- Telegram‑бот:
  ...
  - команда `/year [YYYY]`: без аргумента отчёт за текущий год (YTD), с аргументом — за календарный год, в сообщении показывается явный период и стоимость портфеля на конец периода;
...
- `/year` — приходит summary за год, PNG-график и блок изменений позиций (по умолчанию текущий год YTD).
