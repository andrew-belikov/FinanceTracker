...
- Telegram‑бот:
  ...
  - результат портфеля в `/today`, `/week`, `/month`, monthly PDF и dataset равен изменению стоимости минус только внешние пополнения и выводы; купоны, дивиденды, комиссии, налоги и помеченный налоговый вычет ИИС остаются внутри результата;
  - в последний день месяца JobQueue отправляет monthly PDF через `reporter`, а при сбое reporter использует текстовый month-end fallback;
