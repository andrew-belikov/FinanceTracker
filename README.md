...
- Telegram‑бот:
  ...
  - в дневных/недельных/месячных отчётах показываются доходы (купоны, дивиденды) и расходы (комиссии, налоги); удержанный налог нормализуется в положительный расход, а возврат налога учитывается отдельно;
  ...
  - в последний день месяца JobQueue отправляет monthly PDF через `reporter`, а при сбое reporter использует текстовый month-end fallback; налоговый расход и возврат налога передаются в deterministic/AI facts раздельно;
...
- `income_events` нужен для минутных уведомлений о купонах/дивидендах и части отчётных сумм; его identity включает валюту, а номиналы разных валют не складываются без утверждённого FX-источника;
