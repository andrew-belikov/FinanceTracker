...
- Telegram‑бот:
  ...
  - в дневных/недельных/месячных отчётах показываются доходы (купоны, дивиденды) и расходы (комиссии, налоги); номинальные доходы, налоги и возвраты выводятся отдельными строками для каждой валюты без FX и межвалютного сложения, а `UNKNOWN` сопровождается предупреждением и не входит в базовые итоги; удержанный налог нормализуется в положительный расход, а возврат налога учитывается отдельно;
  - результат портфеля в `/today`, `/week`, `/month`, monthly PDF и dataset равен изменению стоимости минус только внешние пополнения и выводы; дневные payload/dataset относят операции к интервалу `(предыдущая дата снапшота, текущая дата снапшота]`, поэтому gap-day потоки не теряются; купоны, дивиденды, комиссии, налоги и помеченный налоговый вычет ИИС остаются внутри результата;
  - в последний день месяца JobQueue отправляет monthly PDF через `reporter`, а при сбое reporter использует текстовый month-end fallback; налоговый расход и возврат налога передаются в deterministic/AI facts раздельно, включая номинальный `income_by_currency` и признак неизвестной валюты; базовые scalar P&L-показатели остаются только в валюте портфеля;
  ...
  - команда `/invest <sum>` подсказывает, как распределить новое пополнение по таргетам; целые рубли распределяются методом наибольших остатков, поэтому суммы неотрицательны и в точности равны пополнению;
  - команда `/calendar` показывает ожидаемые купоны и объявленные дивиденды по текущим позициям на следующие 90 дней, итоги по месяцам и суммы после расчётного налога; неопубликованный купон оценивается по предыдущей выплате и помечается `~`, а для купонов с надёжным периодом также выводится простой годовой эквивалент чистой выплаты к cost basis;
  - каждый понедельник в `10:00 Europe/Moscow` приходит сводка выплат с понедельника по воскресенье;
  - команда `/twr` с corrected дневным period-first TWR по активному счёту, XIRR в годовых, run-rate на 31 декабря (без новых внешних cashflow) и графиком по дням в двух связанных панелях: стоимость портфеля и TWR; внешний поток суммируется на всём интервале между соседними снапшотами, а TWR выбранного периода rebased к его началу;
  - команда `/dataset` отправляет один ZIP-архив для AI-анализа: внутри `dataset.json`, `daily_timeseries.csv`, `positions_current.csv`, `operations.csv`, `income_events.csv` и `README_AI.md`; summary в архиве period-first, а не lifetime-first, плюс есть `logical_asset_id`, `reconciliation_*` и quality flags; operation cashflows в дневных и периодных фактах разделены по валютам, а P&L снапшота нейтрализует только внешний поток в базовой валюте без неявного FX;
  ...
  - `/year` отправляет 4 сообщения: summary, PNG-график с двумя связанными блоками (сверху — стоимость портфеля на конец месяца, снизу — пополнения за месяц), дополнительный PNG-график `результат по месяцам` (изменение стоимости минус чистый внешний поток `пополнения − выводы` за месяц), и movements (сгруппированный блок изменений позиций 🆕/📈/📉/✅ по первому и последнему снапшоту внутри периода, с агрегацией по логическому активу: ticker → name → figi; валютные позиции из qty-движений исключаются);
  ...
  - авто‑подсказка по каждому новому пополнению счёта: бот автоматически отправляет рекомендацию в логике `/invest <сумма>` и кнопку `Вычет` для обратимой разметки налогового вычета ИИС;
  - event-уведомления и плановые рассылки используют durable recipient ledger: успешный адресат не получает повтор при рестарте или частичном сбое, а общий scheduled run завершается только после доставки всем целевым адресатам; lease планового job можно перехватить только после истечения срока, а finalize/release защищены owner token;
  - утреннее уведомление о новом максимуме портфеля по итогам вчерашнего дня;
  - уведомление о выполнении годового плана пополнений.
- Ограничение доступа по списку доверенных Telegram user_id.
- Актуальное меню команд синхронизируется с Telegram через `setMyCommands` при
  каждом старте `bot`.
...
- `reporter` — отдельный внутренний HTTP‑сервис для PDF pipeline; отдаёт `/healthz`, принимает `POST /reports/monthly/pdf` только от `bot` по `REPORTER_SERVICE_KEY`, ограничивает параллелизм и сроки запросов и добавляет Ollama narrative поверх детерминированных данных с жёстким fallback.
...
Уведомление о фактическом купоне сохраняет доходность выплаты к cost basis и,
если T-Invest передал корректные границы купонного периода, дополнительно
показывает простой годовой эквивалент `ACT/365F`. Это не YTM и не прогноз
будущих купонов; при отсутствии надёжного периода дополнительная строка
пропускается.
...
- `income_events` нужен для минутных уведомлений о купонах/дивидендах и части отчётных сумм; его identity включает валюту, а номиналы разных валют не складываются без утверждённого FX-источника. Пользовательские отчёты и deterministic/AI payload сохраняют раздельные купоны, дивиденды, налоги и возвраты для каждой валюты;
- `migrate --check` не создаёт ledger, таблицы или другие объекты и не применяет DML/DDL: пустая либо отстающая схема завершается ненулевым кодом без изменения БД;
...
- `/today` — приходит сводка; дельта считается между двумя последними снапшотами и вычитает чистый внешний поток на всём интервале `(предыдущая дата, текущая дата]`; lifetime P&L использует чистый внесённый капитал `пополнения − выводы`, а вычет ИИС в него не входит и показывается отдельным доходом.
...
- `/monthpdf` — приходит месячный PDF-отчёт через внутренний `reporter`; допустимы форматы `/monthpdf` и `/monthpdf YYYY MM`. Если `Ollama` недоступна или ответ не проходит валидацию, `reporter` всё равно соберёт PDF на deterministic fallback narrative.
- `/year` — режим YTD (без аргумента): приходит summary за текущий год + 2 PNG-графика (основной и `прирост/падение по месяцам`) + movements (четвёртое сообщение с блоками изменений позиций по группам, агрегация в movements идёт по ticker с fallback на name/figi); график `прирост/падение по месяцам` вычитает чистый внешний поток `пополнения − выводы` за месяц. Если pre-period snapshot отсутствует, первый snapshot периода считается baseline и потоки, уже включённые в него, повторно не вычитаются.
...
- `/history` — строится график стоимости и собственных пополнений; вычеты ИИС отмечаются маркерами на датах поступлений и не влияют на масштаб оси, а максимум портфеля отмечается красной точкой с датой и значением.
- `/structure` — корректно отрисовывается структура портфеля; P&L позиции, группы и итога делится на единый cost basis `текущая стоимость − P&L`.
- `/twr` — приходит corrected дневной period-first TWR по активному счёту, XIRR в годовых, run-rate на 31 декабря без новых пополнений/выводов и график по дням; gap-day потоки входят в следующий доступный интервал снапшотов.
- `/calendar` — приходит календарь ожидаемых купонов и объявленных дивидендов на 90 дней с расчётным налогом и помесячными итогами; неопубликованный купон оценивается по предыдущей выплате и помечается `~`, суммы в разных валютах не складываются.
- `/dataset` — приходит ZIP-архив версии 3; `operations.csv` содержит `cashflow_category`, дневной ряд — `iis_tax_deduction_income`, `total_income_net`, `net_external_flow` и отдельные `*_tax_refunds`, а TWR и внешний поток исключают вычеты ИИС.
- Строковые CSV-ячейки, похожие на spreadsheet formula, получают безопасный текстовый префикс; канонический `dataset.json` сохраняет исходные значения без этой CSV-специфичной обработки.
...
- в последний день месяца приходит текстовый `/month` и monthly PDF; если `reporter` недоступен или PDF не собирается, текстовый отчёт всё равно отправляется, а PDF-доставка остаётся доступной для повторного добора;
- утром приходит уведомление о новом максимуме за вчера, если итоговый снапшот вчерашнего дня превысил предыдущий максимум; при пересечении годового плана приходит соответствующий триггер.
