...
- `reporter` — отдельный внутренний HTTP‑сервис для PDF pipeline; отдаёт `/healthz` и собирает `POST /reports/monthly/pdf`.
- `db` — Postgres.
...
- `/monthpdf` — приходит PDF monthly review через внутренний `reporter`; допустимы форматы `/monthpdf` и `/monthpdf YYYY MM`.
