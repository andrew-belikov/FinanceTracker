...
- Telegram‑бот:
  ...
  - команда `/calendar` показывает ожидаемые купоны и объявленные дивиденды по текущим позициям на следующие 90 дней, итоги по месяцам и суммы после расчётного налога; для купонов с надёжным периодом также выводится простой годовой эквивалент чистой выплаты к cost basis;
...
- `/calendar` — приходит календарь ожидаемых купонов и объявленных дивидендов на 90 дней с расчётным налогом и помесячными итогами; суммы в разных валютах не складываются.
