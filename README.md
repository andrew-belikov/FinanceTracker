...
- Telegram‑бот:
  ...
  - изменение стоимости в `/today`, `/week` и `/month` считается без собственных пополнений и выводов; помеченный налоговый вычет ИИС показывается отдельным доходом и входит в результат;
...
- `/today` — приходит сводка; дневная дельта считается как изменение стоимости минус собственные пополнения/выводы за локальный день, а вычет ИИС входит в результат и показывается отдельным доходом.
...
- `/dataset` — приходит ZIP-архив версии 3; `operations.csv` содержит `cashflow_category`, дневной ряд — `iis_tax_deduction_income` и `total_income_net`, а TWR и внешний поток исключают вычеты ИИС.
