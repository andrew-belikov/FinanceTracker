...
- Telegram‑бот:
  ...
  - `/year` отправляет 3 сообщения: summary, PNG-график с помесячными столбцами (внешний — стоимость портфеля на последний снапшот месяца, внутренний — пополнения за месяц), и movements (сгруппированный блок изменений позиций 🆕/📈/📉/✅ по первому и последнему снапшоту внутри периода, с агрегацией по логическому активу: ticker → name → figi; валютные позиции из qty-движений исключаются);
...
- `/year` — режим YTD (без аргумента): приходит summary за текущий год + PNG-график + movements (третье сообщение с блоками изменений позиций по группам, агрегация в movements идёт по ticker с fallback на name/figi).
- `/year YYYY` — режим календарного года: аналогичный отчёт за указанный год (summary + PNG-график + movements).
