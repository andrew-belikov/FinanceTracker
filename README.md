...
- Telegram‑бот:
  ...
  - `/year` отправляет 3 сообщения: summary, PNG-график с помесячными столбцами (внешний — стоимость портфеля на последний снапшот месяца, внутренний — пополнения за месяц), и movements (сгруппированный блок изменений позиций 🆕/📈/📉/✅ по первому и последнему снапшоту внутри периода);
...
- `/year` — режим YTD (без аргумента): приходит summary за текущий год + PNG-график + movements (третье сообщение с блоками изменений позиций по группам).
