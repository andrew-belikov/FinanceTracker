...
- Telegram‑бот:
  ...
  - `/year` отправляет 4 сообщения: summary, PNG-график с помесячными столбцами (внешний — стоимость портфеля на последний снапшот месяца, внутренний — пополнения за месяц), дополнительный PNG-график `прирост/падение по месяцам` (дельта к предыдущему месяцу по стоимости портфеля), и movements (сгруппированный блок изменений позиций 🆕/📈/📉/✅ по первому и последнему снапшоту внутри периода, с агрегацией по логическому активу: ticker → name → figi; валютные позиции из qty-движений исключаются);
...
- `/year` — режим YTD (без аргумента): приходит summary за текущий год + 2 PNG-графика (основной и `прирост/падение по месяцам`) + movements (четвёртое сообщение с блоками изменений позиций по группам, агрегация в movements идёт по ticker с fallback на name/figi).
