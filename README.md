...
- `/monthpdf` — приходит PDF monthly review через внутренний `reporter`; допустимы форматы `/monthpdf` и `/monthpdf YYYY MM`. Если `Ollama` недоступна или ответ не проходит валидацию, `reporter` всё равно соберёт PDF на deterministic fallback narrative.
