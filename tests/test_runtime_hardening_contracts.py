...
class HandlerBudgetTests(unittest.IsolatedAsyncioTestCase):
    async def test_timeout_keeps_command_budget_until_worker_finishes(self):
        ...
        with ThreadPoolExecutor(max_workers=1) as executor, patch.object(
            handlers, "BOT_COMMAND_TIMEOUT_SECONDS", 0.01
        ), patch.object(handlers, "BOT_COMMAND_MAX_CONCURRENCY", 1), patch.object(
            handlers, "_BOT_COMMAND_EXECUTOR", executor
            ...
            next_task = asyncio.create_task(handlers.run_blocking_command(next_command))
            await asyncio.sleep(0.02)
            self.assertFalse(next_started.is_set())
            release_worker.set()
            self.assertEqual(await next_task, "next")
