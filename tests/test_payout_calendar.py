...
from financetracker.tracker import payout_calendar_sync
...
class PayoutCalendarSyncTests(unittest.TestCase):
    ...
    def test_sync_persists_coupon_and_dividend_amounts_for_current_quantity(self):
        with self.Session() as session:
            ...
            with (
                ...
                stats = payout_calendar_sync.sync_payout_calendar_for_account(
                    session,
                    "account",
                    api_get_bond_coupons=tracker_app.api_get_bond_coupons,
                    api_get_dividends=tracker_app.api_get_dividends,
                    local_today=lambda: date(2026, 7, 28),
                    local_tz=tracker_app.LOCAL_TZ,
                    horizon_days=tracker_app.PAYOUT_CALENDAR_HORIZON_DAYS,
                    dividend_record_lookback_days=tracker_app.PAYOUT_DIVIDEND_RECORD_LOOKBACK_DAYS,
                    logger=tracker_app.logger,
                )
                session.commit()
    ...
    def test_partial_refresh_is_rolled_back_for_failed_instrument(self):
        with self.Session() as session:
            self.add_portfolio(session)
            with (
                mock.patch.object(tracker_app, "local_today", return_value=date(2026, 7, 28)),
                mock.patch.object(tracker_app, "api_get_bond_coupons", return_value=self.coupon_events()),
                mock.patch.object(tracker_app, "api_get_dividends", return_value=[]),
            ):
                tracker_app.sync_payout_calendar_for_account(session, "account")
                session.commit()

            changed_events = self.coupon_events()
            changed_events[0]["payOneBond"]["units"] = "13"
            original_upsert = payout_calendar_sync._upsert_payout_calendar_event
            ...
            with (
                mock.patch.object(tracker_app, "local_today", return_value=date(2026, 7, 28)),
                mock.patch.object(tracker_app, "api_get_bond_coupons", return_value=changed_events),
                mock.patch.object(tracker_app, "api_get_dividends", return_value=[]),
                mock.patch.object(payout_calendar_sync, "_upsert_payout_calendar_event", side_effect=upsert_then_fail),
            ):
                stats = tracker_app.sync_payout_calendar_for_account(session, "account")
                session.commit()

            coupon = session.query(tracker_app.PayoutCalendarEvent).filter_by(event_type="coupon").one()

        self.assertEqual(stats["failed"], 1)
        self.assertEqual(coupon.expected_amount, Decimal("125.00"))
