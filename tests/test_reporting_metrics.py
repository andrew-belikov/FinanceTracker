...
from financetracker.bot import reporting_account


PROJECT_ROOT = Path(__file__).resolve().parents[1]
PERFORMANCE_SERVICE_FILE = PROJECT_ROOT / "src" / "financetracker" / "bot" / "performance_service.py"
...
normalize_reporting_account_id = reporting_account.normalize_reporting_account_id
choose_reporting_account_id = reporting_account.choose_reporting_account_id
resolve_reporting_account_id = reporting_account.resolve_reporting_account_id
...
class ReportingAccountResolutionTests(unittest.TestCase):
    def setUp(self):
        self.original_account_id = reporting_account.TINKOFF_ACCOUNT_ID

    def tearDown(self):
        reporting_account.TINKOFF_ACCOUNT_ID = self.original_account_id
    ...
    def test_resolve_reporting_account_id_prefers_explicit_tinkoff_account_id(self):
        reporting_account.TINKOFF_ACCOUNT_ID = "2271274706"
    ...
    def test_resolve_reporting_account_id_falls_back_to_latest_snapshot(self):
        reporting_account.TINKOFF_ACCOUNT_ID = "auto"
        session = FakeSession("latest-from-db")
