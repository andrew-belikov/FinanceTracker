...
class ArchitectureBoundaryTests(unittest.TestCase):
    ...
    def test_product_packages_are_peer_independent(self):
        ...
        self.assert_package_does_not_import("bot", ("financetracker.reporting", "financetracker.tracker"))

    def test_bot_service_owners_do_not_import_queries_facade(self):
        for name in (
            "performance_service.py",
            "summary_service.py",
            "year_summary_service.py",
            "alerts_service.py",
        ):
            modules = imported_financetracker_modules(ROOT / "bot" / name)
            self.assertNotIn("financetracker.bot.queries", modules, msg=name)
