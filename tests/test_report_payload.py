...
from financetracker.reporting.payload_activity import (  # noqa: E402
    build_operations_top,
    normalize_realized_rows,
)
from financetracker.reporting.payload_timeseries import (  # noqa: E402
    build_instrument_eod_timeseries,
    build_instrument_movers,
)
from financetracker.reporting.payload_summary import (  # noqa: E402
    build_rebalance_snapshot,
    build_reconciliation_rows,
)
from financetracker.reporting.payload_ai_input import build_monthly_ai_input  # noqa: E402
...
class ReportPayloadHelpersTests(unittest.TestCase):
    def test_ai_input_owner_keeps_payload_compatibility_facade(self):
        self.assertIs(report_payload.build_monthly_ai_input, build_monthly_ai_input)

    def test_summary_owner_keeps_payload_compatibility_facade(self):
        self.assertIs(report_payload._build_reconciliation_rows, build_reconciliation_rows)
        self.assertIs(report_payload._build_rebalance_snapshot, build_rebalance_snapshot)
        self.assertEqual(build_reconciliation_rows(None, []), ([], Decimal("0"), Decimal("0")))
        self.assertEqual(build_rebalance_snapshot(None, [], None)["rows"], [])

    def test_activity_owner_keeps_payload_compatibility_facade(self):
        self.assertIs(report_payload.build_operations_top, build_operations_top)
        self.assertIs(report_payload._normalize_realized_rows, normalize_realized_rows)

    def test_timeseries_owner_keeps_payload_compatibility_facade(self):
        self.assertIs(report_payload.build_instrument_eod_timeseries, build_instrument_eod_timeseries)
        self.assertIs(report_payload.build_instrument_movers, build_instrument_movers)
...
class MonthlyReportPayloadBuilderTests(unittest.TestCase):
    ...
    def test_build_monthly_report_payload_matches_published_json_schema(self):
        schema = json.loads(
            (
                PROJECT_ROOT
                / "docs"
        ...
        jsonschema.validate(self._build_payload(), schema)
