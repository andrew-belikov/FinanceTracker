...
from financetracker.reporting import report_render  # noqa: E402
from financetracker.reporting.report_charts import build_monthly_report_charts  # noqa: E402
from financetracker.reporting.report_html import build_monthly_report_html  # noqa: E402
from financetracker.reporting.formatting import display_delta_pp  # noqa: E402
...
class ReportRenderTests(unittest.TestCase):
    def test_chart_owner_keeps_renderer_compatibility_facade(self):
        self.assertIs(report_render.build_monthly_report_charts, build_monthly_report_charts)
        self.assertEqual(
            build_monthly_report_charts({}),
            {"performance": None, "allocation": None, "open_pl": None},
        )

    def test_html_owner_keeps_renderer_compatibility_facade(self):
        self.assertIs(report_render.build_monthly_report_html, build_monthly_report_html)

    def test_artifact_uses_patched_render_facade_builders(self):
        payload = build_sample_payload()
        charts = {"performance": "sentinel", "allocation": None, "open_pl": None}
        narrative = {"schema_version": "test"}
        with (
            mock.patch.object(report_render, "build_monthly_report_charts", return_value=charts),
            mock.patch.object(report_render, "build_deterministic_monthly_narrative", return_value=narrative),
            mock.patch.object(report_render, "build_monthly_report_filename", return_value="test.pdf"),
        ):
            artifact = report_render.build_monthly_report_artifact(
                payload,
                pdf_renderer=lambda html: b"%PDF-test",
            )

        self.assertIs(artifact["charts"], charts)
        self.assertIs(artifact["narrative"], narrative)
        self.assertEqual(artifact["filename"], "test.pdf")
    ...
    def test_display_delta_pp_golden_output(self):
        self.assertEqual(display_delta_pp("6.168", precision=1), "+6,2 п.п.")
        self.assertEqual(display_delta_pp("-2.17", precision=1), "-2,2 п.п.")
