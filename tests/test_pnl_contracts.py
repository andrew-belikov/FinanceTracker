...
SUMMARY_SERVICE_FILE = PROJECT_ROOT / "src" / "financetracker" / "bot" / "summary_service.py"
PAYLOAD_TIMESERIES_FILE = PROJECT_ROOT / "src" / "financetracker" / "reporting" / "payload_timeseries.py"
...
class DailyPortfolioResultTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.build_rows = staticmethod(load_function(PAYLOAD_TIMESERIES_FILE, "build_timeseries_daily", namespace))
        cls.compute_period_pnl = staticmethod(load_function(PAYLOAD_TIMESERIES_FILE, "compute_period_pnl", namespace))
...
class TodaySummaryContractTests(unittest.TestCase):
    def _build(self, snapshots, *, expected_interval_flow, deposits=100, net_contributions=50):
        ...
        build_today = load_function(SUMMARY_SERVICE_FILE, "build_today_summary", namespace)
...
class StructurePnlDenominatorTests(unittest.TestCase):
    def _render(self, positions):
        ...
        build_structure = load_function(SUMMARY_SERVICE_FILE, "build_structure_text", namespace)
