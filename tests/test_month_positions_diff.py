...
def load_compute_positions_diff_lines():
    module_ast = ast.parse(SUMMARY_SERVICE_FILE.read_text(encoding="utf-8"), filename=str(SUMMARY_SERVICE_FILE))
    target_node = None
    ...
    code = compile(isolated_module, filename=str(SUMMARY_SERVICE_FILE), mode="exec")
    namespace = {}
    exec(code, namespace)
