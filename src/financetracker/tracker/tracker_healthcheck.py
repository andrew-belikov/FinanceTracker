...
from financetracker.common.readiness_state import process_state, validate_ready_state
from financetracker.common.runtime_paths import configured_runtime_file, default_runtime_file


DEFAULT_READY_FILE = default_runtime_file("financetracker-tracker.ready")
...
def main(
    *,
    ...
    try:
        ...
        return 1
    if (
        b"financetracker-tracker" not in command_line
        and b"app.py" not in command_line
    ) or owner_state in {None, "Z", "X"}:
        return 1
    ready_path = state_path or configured_runtime_file("TRACKER_READY_FILE", "financetracker-tracker.ready")
