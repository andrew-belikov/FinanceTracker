...
from financetracker.database.schema_manifest import manifest_digest, validate_schema_manifest
...
def _load_database_runtime():
    # Importing the module registers every SQLAlchemy model in the shared metadata.
    _ = database_models.Instrument
    engine, _session_local = create_session_factory(read_database_settings().dsn)
    return Base, engine
...
def _run_check_only(connection, Base, migrations: list[Path]) -> int:
    ...
    schema_drift = validate_schema_manifest(connection)
    if schema_drift:
        raise MigrationError("Schema manifest drift: " + "; ".join(schema_drift))
    logger.info(
        ...
        "Database migrations are up to date.",
        {
            ...
            "schema_manifest_sha256": manifest_digest(
                tuple(path.name for path in migrations)
            ),
        },
    )
    return 0
