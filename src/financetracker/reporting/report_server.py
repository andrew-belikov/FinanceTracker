...
from financetracker.reporting.report_pipeline import (
    ReportRequestError,
    build_monthly_report_artifact_for_request,
    build_report_health_payload,
)
from financetracker.reporting.report_render import ReportRenderError


# The HTTP service is addressed by bot only through bot_reporter_internal; its
# Compose definition deliberately uses `expose`, never a host `ports` mapping.
INTERNAL_REPORTER_LISTEN_HOST = "0.0.0.0"  # nosec B104
REPORTER_HOST = os.getenv("REPORTER_HOST", INTERNAL_REPORTER_LISTEN_HOST).strip() or INTERNAL_REPORTER_LISTEN_HOST
