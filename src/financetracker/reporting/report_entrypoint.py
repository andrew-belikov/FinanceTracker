...
from financetracker.common.logging_setup import configure_logging
...
def _run_healthcheck() -> int:
    # Healthchecks must only call the reporter process over loopback.
    with urlopen(f"http://127.0.0.1:{REPORTER_PORT}/healthz", timeout=2) as response:  # nosec B310
...
def main() -> int:
    ...
    from financetracker.reporting.report_server import main as run_reporter_server
