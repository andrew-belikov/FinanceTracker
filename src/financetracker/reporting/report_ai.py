...
from financetracker.reporting.report_payload import build_monthly_ai_input
...
def build_ollama_chat_url(base_url: str) -> str:
    """Build the only permitted Ollama endpoint from an HTTP(S) base URL."""
    parsed = urlsplit(base_url.strip())
    if parsed.scheme not in {"http", "https"} or not parsed.hostname:
        raise ReportAIError("OLLAMA_BASE_URL must be an absolute HTTP(S) URL.")
    if parsed.username is not None or parsed.password is not None:
        raise ReportAIError("OLLAMA_BASE_URL must not contain credentials.")
    path = parsed.path.rstrip("/") + "/api/chat"
    return urlunsplit((parsed.scheme, parsed.netloc, path, "", ""))


def _call_ollama_chat(messages: list[dict[str, str]]) -> dict[str, Any]:
    url = build_ollama_chat_url(OLLAMA_BASE_URL)
    body = json.dumps(
        {
            "model": OLLAMA_MODEL,
            "stream": False,
    ...
    try:
        # The request URL is restricted to an absolute credential-free HTTP(S) endpoint.
        with request.urlopen(http_request, timeout=OLLAMA_TIMEOUT_SECONDS) as response:  # nosec B310
