...
from financetracker.bot.queries import (
...
from financetracker.domain.assets import build_asset_alias_lookup, build_reconciliation_by_asset_type
...
from financetracker.domain.operations import classify_operation_group, is_income_event_backed_tax_operation
...
def build_dataset_export(session) -> tuple[dict, list[dict], list[dict], list[dict], list[dict]]:
    ...
    operations_rows = list(
        # Operation instants are UTC-aware; dataset bounds represent local civil days.
        get_dataset_operations(
            session,
            account_id=account_id,
            start_dt=local_reporting_bounds_utc(
                min_date,
                max_date + timedelta(days=1),
            )[0],
            end_dt=local_reporting_bounds_utc(
                min_date,
                max_date + timedelta(days=1),
            )[1],
        )
