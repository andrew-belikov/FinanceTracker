...
# Compatibility re-export; new repository code imports the database owner directly.
from financetracker.database.operations import OPERATIONS_DEDUP_CTE  # noqa: F401
...
def to_local_market_date(dt: datetime | None) -> date | None:
    if dt is None:
        return None
    return utc_to_local_date(dt, TZ)


def local_reporting_bounds_utc(
    start: date | datetime,
