...
from financetracker.common.readiness_state import validate_ready_state
from financetracker.common.runtime_paths import configured_runtime_file, default_runtime_file


DEFAULT_READY_FILE = default_runtime_file("financetracker-bot.ready")
...
def main(
    *,
    ...
    try:
        ...
        return 1
    if b"financetracker-bot" not in owner_command and b"entrypoint.py" not in owner_command:
        return 1
    ready_path = state_path or configured_runtime_file("BOT_READY_FILE", "financetracker-bot.ready")
    ...
    if not valid:
        return 1
    if (
        os.getenv("APP_ENV", "").strip().lower() == "ci"
        and os.getenv("BOT_STARTUP_MODE", "").strip() == "container-smoke"
    ):
        return 0
    return 0 if any(
        "financetracker.bot.bot" in command_line or " bot.py" in f" {command_line}"
