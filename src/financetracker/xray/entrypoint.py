...
from financetracker.common.logging_setup import configure_logging, get_logger, relay_text_stream
...
STATUS_FILE = configured_runtime_file("XRAY_STATUS_FILE", "xray-client-status.json")
CONFIG_FILE = configured_runtime_file("XRAY_CONFIG_FILE", "xray-client-config.json")
...
def xray_output_level(line: str) -> int:
    """Keep exceptional Xray child output visible without connection-log flood."""
    normalized = line.lower()
    if any(marker in normalized for marker in (" error", "[error]", "failed", "failure", "fatal")):
        return logging.ERROR
    if any(marker in normalized for marker in (" warning", " warn", "[warning]", "[warn]")):
    ...
    if proc.stdout is not None:
        relay_threads.append(
            relay_text_stream(
                ...
                level_for_line=xray_output_level,
            )
        )
