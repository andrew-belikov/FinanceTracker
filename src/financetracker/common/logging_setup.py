...
# Password in a URI user-info component, for example a database DSN.  This is
# applied to every emitted string, including exception messages and tracebacks.
_RE_URI_PASSWORD = _re.compile(
    r"((?:[a-z][a-z0-9+.-]*://)[^\s/@:]+:)([^\s/@]+)(@)",
    _re.IGNORECASE,
)
...
def _sanitize_string(s: str) -> str:
    ...
    s = _RE_URI_PASSWORD.sub(r"\1" + _REDACTED + r"\3", s)
    return s
...
def _is_sensitive_key(key: str, value: Any) -> bool:
    normalized = key.strip().lower()
    # ``error`` is a required structured diagnostics object.  Its string
    # leaves are still sanitized recursively below, but redacting the whole
    # object hides exception type and location needed for incident response.
    if normalized == "error" and isinstance(value, Mapping):
        return False
    if normalized == "error":
        return True
...
def relay_text_stream(
    ...
    *,
    ...
    level_for_line: Callable[[str], int] | None = None,
) -> _threading.Thread:
    ...
    def _relay() -> None:
        try:
            for raw_line in iter(stream.readline, ""):
                ...
                logger.log(level_for_line(line) if level_for_line else level, event, line, line_ctx)
        except Exception:
