...
- выполняет `compileall`, Ruff-проверку `E9,F`, Bandit-проверку medium-and-higher,
  ...
  guardrail размера крупных multi-purpose modules,
...
Текущая официальная версия — `1.0.0`. Используйте SemVer в формате
`MAJOR.MINOR.PATCH`: `PATCH` — исправление без изменения контракта, `MINOR` —
обратно-совместимая возможность, `MAJOR` — несовместимое изменение. Перед релизом обновите `CHANGELOG.md`, пройдите CI и
