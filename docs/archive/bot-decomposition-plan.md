# Архив: Bot Decomposition Plan

> План относится к прежнему flat layout `src/bot`. Его шаги уже не являются
> инструкцией к изменению current package-layout; используйте
> [`../ARCHITECTURE.md`](../ARCHITECTURE.md) и [`../TARGET_ARCHITECTURE.md`](../TARGET_ARCHITECTURE.md).

## Цель

Поэтапно распилить [`src/bot/bot.py`](../../src/bot/bot.py) на отдельные модули без изменения внешнего поведения бота.
...
- Не переходить на package-style imports вида `from bot.handlers import ...`.
  Причина: контейнер из [`docker/Dockerfile.bot`](../../docker/Dockerfile.bot) копирует `src/bot/*` плоско в `/app` и запускает `python bot.py`.
- Использовать sibling-imports: `from runtime import ...`, `from queries import ...`.
- `src/bot/bot.py` должен остаться entrypoint-композитором до самого конца, потому что [`src/bot/entrypoint.py`](../../src/bot/entrypoint.py) делает `os.execvp("python", ["python", "-u", "bot.py"])`.
...
5. Обновить [`tests/test_runtime_logging_guardrails.py`](../../tests/test_runtime_logging_guardrails.py): добавить `runtime.py` в `RUNTIME_FILES`.
...
5. Обновить [`tests/test_reporting_metrics.py`](../../tests/test_reporting_metrics.py):
...
4. Перевести AST-based tests:
   - [`tests/test_reporting_metrics.py`](../../tests/test_reporting_metrics.py): TWR math из `services.py`
   - [`tests/test_rebalance_features.py`](../../tests/test_rebalance_features.py): rebalance helpers и `build_help_text` из `services.py`
   - [`tests/test_month_positions_diff.py`](../../tests/test_month_positions_diff.py): `compute_positions_diff_lines` из `services.py`
...
4. Обновить [`tests/test_dataset_helpers.py`](../../tests/test_dataset_helpers.py), чтобы он читал symbols из `dataset.py`, а не из `bot.py`.
...
6. Обновить [`tests/test_runtime_logging_guardrails.py`](../../tests/test_runtime_logging_guardrails.py): добавить `jobs.py` в `RUNTIME_FILES`.
...
5. `register_handlers(app)` должен зарегистрировать:
   ...
   - все `CommandHandler(...)`
6. Обновить [`tests/test_runtime_logging_guardrails.py`](../../tests/test_runtime_logging_guardrails.py): добавить `handlers.py` в `RUNTIME_FILES`.
...
4. Обновить [`docs/ARCHITECTURE.md`](../ARCHITECTURE.md):
   - зафиксировать новую модульную структуру бота
   - кратко описать границы модулей
5. При необходимости кратко обновить [`README.md`](../../README.md), если там появятся упоминания внутренней структуры разработки.
