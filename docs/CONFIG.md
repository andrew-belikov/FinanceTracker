...
- Для операций используется временная проверка через таблицу `deposits` (sanity-check):
...
- При переходе на отдельную таблицу `operations` нужно обновить только источник этой проверки в боте.
