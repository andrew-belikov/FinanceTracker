...
Поддерживаемые операторские параметры перечислены в [`.env.example`](../.env.example).
Скопируйте его в `.env`, задайте секреты и не добавляйте полученный файл в Git.
...
- `TIMEZONE` — основная бизнес-таймзона (например, `Europe/Moscow`). Она
  определяет отображение дат, локальные отчётные периоды, JobQueue `bot`,
  локальную дату `tracker` и ежедневный sync календаря выплат. Границы
  локального полуинтервала `[начало, конец)` переводятся в UTC перед запросом к
  `TIMESTAMPTZ`; дневная и месячная группировка выполняется обратно в этой
  таймзоне.
- `SCHED_TZ` — таймзона APScheduler процесса `tracker` и cron-снапшота; по
  умолчанию `Europe/Moscow`. Если используется другая `TIMEZONE`, задайте
  `SCHED_TZ` тем же значением, чтобы все календарные задания были согласованы.
...
- `TINKOFF_ACCOUNT_ID` — фиксированный `account_id`, для которого требуется
  точное совпадение. При пустом значении или `auto` должен существовать ровно один открытый счёт;
  при нуле или нескольких открытых счетах `tracker` останавливается без записи.
...
- `PAYOUT_CALENDAR_SYNC_HOUR` / `PAYOUT_CALENDAR_SYNC_MINUTE` — ежедневное время
  обновления календаря `tracker` в `TIMEZONE` (по умолчанию `09:00`).
...
- `ALLOW_INSECURE_TLS_FOR_TESTS` — test-only break-glass для
  `VERIFY_SSL=false`; production отклоняет отключение TLS-проверки независимо
  от этого флага.
...
- `OLLAMA_ENABLED` — включает narrative-layer через локальную `Ollama`
  (`true/false`). По умолчанию `false`.
- `OLLAMA_BASE_URL` — базовый URL `Ollama` из контейнера `reporter`; при общей
  Docker-сети используйте `http://ollama:11434`, а не `localhost`.
...
- При `BOT_PROXY_ENABLED=false` контейнер `bot` запускается без
  `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY` и обращается к Telegram напрямую.
...
Если `Ollama` запущена отдельным compose-проектом в сети
`localllm_localllm`, для `reporter` нельзя использовать `localhost`.
...
- `SNAPSHOT_MODE` — режим запуска `tracker`: `interval` (по умолчанию) или
  `cron`.
- `SNAPSHOT_INTERVAL_MINUTES` — интервал между снапшотами в режиме `interval`
  (по умолчанию `5`).
- `SNAPSHOT_HOUR`, `SNAPSHOT_MINUTE` — локальное время снапшота в режиме `cron`;
  используется таймзона `SCHED_TZ`.

Повторный снапшот за тот же локальный день обновляет дневное состояние. Точный
контракт хранения и синхронизации операций описан в
[contracts/README.md](contracts/README.md), пользовательское поведение — в
